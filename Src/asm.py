...
* порождает на выходе питон файл 
    * с кодом (модуль)
    * с юнит-тестами для pytest (параметр `what` функции processFile())

Из имени входного файла удаляется суффикс `_asm` (т.е. из файла asm_smoke_asm.ipynb будет порожден модуль asm_smoke.py)
...
См. юнит-тесты и пример `asm_smoke_asm.ipynb` в директории Examples.
